...
export function useToast({ defaultOptions }: UseToastParams) {
	...
	const initialOptions = mergeIfDefined(DEFAULT_OPTIONS, defaultOptions) as Required<ToastOptions>
	const [options, setOptions] = React.useState<Required<ToastOptions>>(initialOptions)
